...
public class AddOrderActivity extends AppCompatActivity {
    ...
    private Spinner carSpinner;
    private LinearLayout selectedServicesContainer;
    private EditText dateInput, commentInput;
    private LinearLayout timeSlotsContainer;
    private TextView timeSlotsTitle;
    private Button bookButton, clearButton;
    private TextView totalPriceView;
    ...
    private SharedPreferences sharedPreferences;
    private CarViewModel carViewModel;
    private OrdersRepository ordersRepository; // Объявляем репозиторий для заказов

    private String selectedTimeSlot = "";
    private List<String> bookedTimeSlots = new ArrayList<>();
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        carViewModel = new ViewModelProvider(this).get(CarViewModel.class);
        // Инициализация OrdersRepository
        ordersRepository = new OrdersRepository(ApiClient.getInstance());
    ...
    private void initViews() {
        ...
        timeSlotsTitle = findViewById(R.id.timeSlotsTitle);
        bookButton = findViewById(R.id.bookButton);
        clearButton = findViewById(R.id.clearServicesButton);
        totalPriceView = findViewById(R.id.totalPriceView);
        sharedPreferences = getSharedPreferences("OrderPrefs", MODE_PRIVATE);
    ...
    private void setupDateTimePickers() {
        dateInput.setOnClickListener(v -> {
            new DatePickerDialog(this, (view, year, month, day) -> {
                calendar.set(year, month, day);
                updateDateInput();
                loadBookedTimeSlotsForDate(calendar);
            },
                    calendar.get(Calendar.YEAR),
                    calendar.get(Calendar.MONTH),
                    calendar.get(Calendar.DAY_OF_MONTH))
    ...
    private void loadBookedTimeSlotsForDate(Calendar date) {
        ...
        Log.d(TAG, "Загрузка занятых слотов для даты: " + formattedDate);
        ...
        try {
            ...
            // Используем OrdersRepository для получения занятых слотов
            ordersRepository.getBookedTimeSlots(token, formattedDate, new OrdersRepository.BookedTimeSlotsCallback() {
                @Override
                public void onSuccess(List<String> slots) {
                    runOnUiThread(() -> {
                        bookedTimeSlots = slots;
                        generateTimeSlots(bookedTimeSlots);
                    });
                }

                @Override
                public void onError(String error) {
                    Log.e(TAG, "Ошибка загрузки временных слотов: " + error);
                    runOnUiThread(() -> {
                        showToast("Ошибка загрузки временных слотов: " + error);
                        // В случае ошибки, генерируем все слоты как свободные
            ...
            Log.e(TAG, "Ошибка при получении токена для занятых слотов", e);
            showToast("Ошибка при получении данных о занятых слотах");
            bookedTimeSlots.clear();
            generateTimeSlots(bookedTimeSlots);
        }
    }

//    private void generateTimeSlots(List<String> bookedSlots) {
//        timeSlotsContainer.removeAllViews();
//        timeSlotsContainer.setVisibility(View.VISIBLE);
//        timeSlotsTitle.setVisibility(View.VISIBLE);
//
//        selectedTimeSlot = "";
//
//// Формат для сравнения с сервером (с секундами)
...
//
//        Log.d(TAG, "Занятые слоты с сервера: " + bookedSlots);
//
//        for (int hour = 9; hour < 20; hour++) {
//            for (int minute = 0; minute < 60; minute += 30) {
//                slotCalendar.set(Calendar.HOUR_OF_DAY, hour);
//                slotCalendar.set(Calendar.MINUTE, minute);
//                slotCalendar.set(Calendar.SECOND, 0); // Явно устанавливаем секунды в 0
//
//                // Полный формат времени для проверки занятости
//                String serverTime = serverFormat.format(slotCalendar.getTime()); // "14:00:00"
//                // Формат для отображения пользователю
//                String displayTime = displayFormat.format(slotCalendar.getTime()); // "14:00"
//
//                if (currentRow == null || slotsInCurrentRow >= SLOTS_PER_ROW) {
//                    currentRow = createNewTimeSlotRow();
//                    timeSlotsContainer.addView(currentRow);
//                    slotsInCurrentRow = 0;
//                }
//
//                Button timeSlotButton = new Button(this);
//                timeSlotButton.setText(displayTime); // Показываем без секунд
//                timeSlotButton.setTag(serverTime);   // Но в теге храним полное время
//
//                setupTimeSlotButtonLayout(timeSlotButton, slotsInCurrentRow);
//
//                boolean isBooked = bookedSlots.contains(serverTime);
//                Log.d(TAG, "Слот " + serverTime + " (" + displayTime + ") - занят: " + isBooked);
//
//                if (isBooked) {
//                    timeSlotButton.setBackgroundColor(Color.parseColor("#CCCCCC"));
//                    timeSlotButton.setTextColor(Color.parseColor("#888888"));
//                    timeSlotButton.setEnabled(false);
//                } else {
//                    timeSlotButton.setBackgroundColor(Color.parseColor("#2260FF"));
//                    timeSlotButton.setTextColor(Color.WHITE);
//                    timeSlotButton.setEnabled(true);
//                    timeSlotButton.setOnClickListener(v -> {
//                        handleTimeSlotSelection(v);
//                        showToast("Выбрано время: " + displayTime); // Показываем пользователю без секунд
//                    });
//                }
//
//                currentRow.addView(timeSlotButton);
//                slotsInCurrentRow++;
//            }
//        }
//    }
...
//    private LinearLayout createNewTimeSlotRow() {
...
//    }
//
//    private void setupTimeSlotButtonLayout(Button button, int slotPosition) {
//        LinearLayout.LayoutParams params = new LinearLayout.LayoutParams(
//                0,
//                LinearLayout.LayoutParams.WRAP_CONTENT,
//                1.0f
...
//    }
//
//    private void handleTimeSlotSelection(View selectedView) {
//        if (!selectedTimeSlot.isEmpty()) {
//            Button prevSelected = timeSlotsContainer.findViewWithTag(selectedTimeSlot);
//            if (prevSelected != null) {
//                prevSelected.setBackgroundColor(Color.parseColor("#2260FF"));
//            }
//        }
//        selectedTimeSlot = (String) selectedView.getTag();
//        selectedView.setBackgroundColor(Color.parseColor("#1A4BCC"));
//    }
    ...
    private void generateTimeSlots(List<String> bookedSlots) {
        ...
        Calendar slotCalendar = (Calendar) calendar.clone();
        LinearLayout currentRow = null;
        final int SLOTS_PER_ROW = 2; // 2 слота в строке
        ...
        for (int hour = 9; hour < 20; hour++) {
            for (int minute = 0; minute < 60; minute += 30) {
                ...
                String serverTime = serverFormat.format(slotCalendar.getTime());
                String displayTime = displayFormat.format(slotCalendar.getTime());

                // Создание новой строки при необходимости
                if (currentRow == null || currentRow.getChildCount() >= SLOTS_PER_ROW) {
                    currentRow = new LinearLayout(this);
                    currentRow.setOrientation(LinearLayout.HORIZONTAL);
                    LinearLayout.LayoutParams rowParams = new LinearLayout.LayoutParams(
                            LinearLayout.LayoutParams.MATCH_PARENT,
                            LinearLayout.LayoutParams.WRAP_CONTENT);
                    rowParams.setMargins(0, 0, 0, 16); // Отступ снизу
                    currentRow.setLayoutParams(rowParams);
                    timeSlotsContainer.addView(currentRow);
                }

                // Создание кнопки слота
                Button timeSlotButton = new Button(this);
                timeSlotButton.setText(displayTime);
                timeSlotButton.setTag(serverTime);

                // Настройка внешнего вида кнопки
                GradientDrawable shape = new GradientDrawable();
                shape.setShape(GradientDrawable.RECTANGLE);
                shape.setCornerRadius(24f); // Скругление углов

                LinearLayout.LayoutParams params = new LinearLayout.LayoutParams(
                        0,
                        dpToPx(48), // Фиксированная высота 48dp
                        1.0f);
                params.setMargins(dpToPx(8), 0, dpToPx(8), 0); // Отступы по бокам

                timeSlotButton.setLayoutParams(params);
                timeSlotButton.setPadding(0, 0, 0, 0);
                timeSlotButton.setAllCaps(false);
                timeSlotButton.setTextSize(TypedValue.COMPLEX_UNIT_SP, 16);
                timeSlotButton.setTypeface(null, Typeface.BOLD);

                // Проверка занятости слота
                boolean isBooked = bookedSlots.contains(serverTime);
                if (isBooked) {
                    // Стиль для занятых слотов
                    shape.setColor(Color.parseColor("#66AACDFF")); // Основной цвет
                    timeSlotButton.setTextColor(Color.parseColor("#2260FF")); // Цвет текста
                    timeSlotButton.setEnabled(false);
                } else {
                    // Стиль для свободных слотов
                    shape.setColor(Color.parseColor("#2260FF")); // Основной цвет
                    timeSlotButton.setTextColor(Color.WHITE); // Цвет текста
                    timeSlotButton.setEnabled(true);
                    timeSlotButton.setOnClickListener(v -> {
                        handleTimeSlotSelection(v);
                        showToast("Выбрано время: " + displayTime);
                    });
                }

                timeSlotButton.setBackground(shape);
                currentRow.addView(timeSlotButton);
            }
        }
    }
    ...
    private void handleTimeSlotSelection(View selectedView) {
        ...
        for (int i = 0; i < timeSlotsContainer.getChildCount(); i++) {
            LinearLayout row = (LinearLayout) timeSlotsContainer.getChildAt(i);
            for (int j = 0; j < row.getChildCount(); j++) {
                View child = row.getChildAt(j);
                if (child instanceof Button && child.getTag().equals(selectedTimeSlot)) {
                    GradientDrawable shape = new GradientDrawable();
                    shape.setShape(GradientDrawable.RECTANGLE);
                    shape.setCornerRadius(24f);
                    shape.setColor(Color.parseColor("#2260FF"));
        ...
        selectedShape.setCornerRadius(24f);
        selectedShape.setColor(Color.parseColor("#1A4BCC")); // Цвет выбранного слота
        selectedView.setBackground(selectedShape);
    }
    ...
    private void submitOrder() {
        ...
        if (dateInput.getText().toString().isEmpty()) {
            Log.w(TAG, "Не указана дата");
            showToast("Укажите дату");
            return;
        }

        if (selectedTimeSlot.isEmpty()) {
            Log.w(TAG, "Не выбрано время");
        ...
        try {
            ...
            String orderDate = new SimpleDateFormat("yyyy-MM-dd", Locale.getDefault()).format(calendar.getTime());
            order.put("order_date", orderDate);
            order.put("order_time", selectedTimeSlot);
            order.put("comment", commentInput.getText().toString());
            order.put("total_price", totalPrice);
            order.put("services", servicesArray);

            Log.d(TAG, "Формируемый заказ: " + order.toString());
            // Используем прямой вызов API через клиент, а не через репозиторий для создания заказа,
            // так как репозиторий не предоставляет метод для создания заказа с JSONObject.
            // Если вы хотите использовать репозиторий, вам нужно будет добавить метод createOrder с JSONObject в OrdersRepository.
            sendOrder(order, token);
        } catch (Exception e) {
            Log.e(TAG, "Ошибка создания заказа", e);
            showToast("Ошибка создания заказа: " + e.getMessage());
    ...
    private void sendOrder(JSONObject order, String token) {
        // Здесь мы используем apiClient напрямую, так как ваш OrdersRepository
        // не предоставляет метод для отправки JSONObject для создания заказа.
        // Если вы добавите такой метод в OrdersRepository, вы можете использовать его здесь.
        OkHttpClient client = null;
        try {
            client = ApiClient.getInstance().getClient();
        } catch (Exception e) {
            Log.e(TAG, "Ошибка получения ApiClient: " + e.getMessage());
        ...
        Request request = new Request.Builder()
                .url(ApiClient.getBaseUrl() + "orders") // Используем базовый URL из ApiClient
                .addHeader("Authorization", "Bearer " + token)
                .post(body)
                .build();
        ...
        client.newCall(request).enqueue(new okhttp3.Callback() { // Используем полный путь к Callback
            @Override
            public void onFailure(Call call, IOException e) {
                Log.e(TAG, "Ошибка сети", e);
                runOnUiThread(() -> showToast("Ошибка соединения: " + e.getMessage()));
            ...
            @Override
            public void onResponse(Call call, okhttp3.Response response) throws IOException { // Используем полный путь к Response
                String responseBody = response.body() != null ? response.body().string() : "null";
                Log.d(TAG, "Ответ сервера. Код: " + response.code() + ", Тело: " + responseBody);
                ...
                runOnUiThread(() -> {
                    try {
                        if (response.isSuccessful()) {
                            ...
                            timeSlotsTitle.setVisibility(View.GONE);
                            dateInput.setText("");
                            finish();
                        } else {
                            handleServerError(response.code(), responseBody);
    ...
    private void handleServerError(int code, String responseBody) {
        try {
            ...
            switch (code) {
                ...
                case 409: // Добавлен код 409 для конфликта (слот занят)
                    Log.w(TAG, "Ошибка 409: " + errorMessage);
                    showToast(errorMessage); // Показываем сообщение о занятости слота
                    break;
